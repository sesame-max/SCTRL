...
#define NRF_CE_LOW(GPIO_PORT, GPIO_PIN) HAL_GPIO_WritePin((GPIO_TypeDef *)GPIO_PORT, (uint16_t)GPIO_PIN, GPIO_PIN_RESET)
#define NRF_CE_HIGH(GPIO_PORT, GPIO_PIN) HAL_GPIO_WritePin((GPIO_TypeDef *)GPIO_PORT, (uint16_t)GPIO_PIN, GPIO_PIN_SET)
#define NRF_CS_LOW(GPIO_PORT, GPIO_PIN) HAL_GPIO_WritePin((GPIO_TypeDef *)GPIO_PORT, (uint16_t)GPIO_PIN, GPIO_PIN_RESET)
#define NRF_CS_HIGH(GPIO_PORT, GPIO_PIN) HAL_GPIO_WritePin((GPIO_TypeDef *)GPIO_PORT, (uint16_t)GPIO_PIN, GPIO_PIN_SET)
...
#define NRF_FIFO_STATUS 0x17   // FIFO状态寄存器;bit0,RX FIFO寄存器空标志;bit1,RX FIFO满标志;bit2,3,保留
                               // bit4,TX FIFO空标志;bit5,TX FIFO满标志;bit6,1,循环发送上一数据包.0,不循
#define NRF_RX_FIFO_EMPTY 0x01 // RX FIFO空标志
#define NRF_TX_FIFO_EMPTY 0x10 // TX FIFO空标志
typedef struct __NRF24L01TypeDef NRF24L01TypeDef;

struct __NRF24L01TypeDef
{
    uint32_t spi;
    uint8_t *rxBuffer;
    uint32_t packLen;
    ...
    uint32_t cs_port;
    uint32_t cs_pin;
    void (*rxCallback)(NRF24L01TypeDef *dev);
    void (*txCallback)(NRF24L01TypeDef *dev);
};
